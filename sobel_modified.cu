/* Compile and run: nvcc -arch=sm_35 -I/usr/local/include/opencv2/ `pkg-config --cflags opencv` -o sobel_modified sobel_modified.cu -I/usr/local/include/opencv2/ `pkg-config opencv --cflags --libs` -lopencv_gpu -lopencv_core -lopencv_highgui
 */
...
int main(int argc, const char* argv[])
{
	...
	for (int i = 0; i < num_tests; ++i) {
		...
		cv::Ptr<cv::cuda::Filter> filter = cv::cuda::createSobelFilter(input_cuda.type(), output_cuda.type(), 1, 1);
		...
		cv::Sobel(bgr_planes[1], output_cpu, CV_8UC1, 1, 1);
		clock_t tEnd3 = clock();
